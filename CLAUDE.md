...
**Phase 1.5** — deepening the bo3 dataset before adding a new source.
Phases 0 and 1 have shipped: the Kalshi collector runs on a schedule, and
bo3 fixtures resolve against Kalshi events at a ~46/47 bind rate.

In scope right now:

```
scripts/phase1_sync.py        map-level ingest via bo3.games()
scripts/backfill.py           12-month history (new)
.github/workflows/sync.yml    schedule phase1_sync (new)
```

Do **not** add HLTV scrapers, Flask routes, or `stats/` modules yet. If a task
seems to need one, say so rather than building it — the phase order exists
because each phase's data makes the next one meaningful.
